package com.juane.model;

public class RemoteConstants {
	// Others
	public static final String EXTERNAL_TOOL_COMMANDS_DIRECTORY = "mylibs";

	// Messages to stablish connection
	public static final String WELCOME_CLIENT_MESSAGE = "welcome";
	public static final String BYE_CLIENT_MESSAGE = "bye";

	// commands
	public static final String EXTERN_TOOL_COMMAND = "nircmdc.exe";
	public static final String SYSVOLUME_COMMAND = "setsysvolume";
	public static final String KEYPRESS_COMMAND = "sendkeypress";
	public static final String SENDKEY_COMMAND = "sendkey";
	public static final String CMD_COMMAND = "cmd";

	// commands parameters
	public static final String LEFT_KEY_PARAM = "left";
	public static final String RIGHT_KEY_PARAM = "right";
	public static final String SPC_KEY_PARAM = "spc";
	public static final String DOWN_PARAM = "down";
	public static final String CMD_C_PARAM = "\\c";

}
